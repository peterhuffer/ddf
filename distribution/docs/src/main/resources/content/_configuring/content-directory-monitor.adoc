...
[TIP]
====
If the CDM directory being monitored is inside ${home_directory}, then adding permissions is not required and this section can be skipped.
====
...
The CDM processes files in a directory, and all of its sub-directories. The CDM offers two options:
