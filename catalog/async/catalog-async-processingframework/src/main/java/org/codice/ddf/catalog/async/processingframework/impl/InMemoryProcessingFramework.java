        ...
        for (ProcessResourceItem item : processRequest.getProcessItems()) {
            ...
            if (processResource != null && processResource.isModified()) {
                try {
                    ...
                    ContentItem contentItem = new ContentItemImpl(processResource.getId(),
                            processResource.getQualifier(),
                            byteSource,
                            processResource.getMimeTypeRawData(),
    ...
    private void storeContentItemUpdates(Set<ContentItem> contentItemsToUpdate,
        ...
        if (CollectionUtils.isNotEmpty(contentItemsToUpdate)) {
            ...
            UpdateStorageRequest updateStorageRequest =
                    new UpdateStorageRequestImpl(new ArrayList<>(contentItemsToUpdate),
                            putPostProcessCompleteFlagAndGet(properties));
    ...
    private void copyItemStream(ProcessResource processResource) {
        try {
            ...
            if (processResource instanceof ProcessResourceImpl) {
                ...
                addToResourceMap(processResource.getId(), outputStream);
            }
        } catch (IOException e) {
